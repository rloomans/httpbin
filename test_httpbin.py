...
def _make_digest_auth_header(
    ...
    auth_header = (
        'Digest username="{0}", response="{1}", uri="{2}", nonce="{3}"'.format(
            username, auth_response, uri, nonce
        )
    )
...
class HttpbinTestCase(unittest.TestCase):
    ...
    def test_get(self):
        ...
        # self.assertEqual(data['headers']['Content-Length'], '0') ## todo: investigate why content-length should be in the request / why it is not
    ...
    def test_post_body_unicode(self):
        ...
        self.assertEqual(
            json.loads(response.data.decode("utf-8"))["data"], "оживлённым"
        )
    ...
    def test_headers(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            {"Accept", "Host", "User-Agent"}.issubset(
                set(response.json["headers"].keys())
            )
        )
    ...
    def test_headers_show_env(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            {"Accept", "Host", "User-Agent", "Via"}.issubset(
                set(response.json["headers"].keys())
            )
        )
            ...
            for algorithm in None, "MD5", "SHA-256", "SHA-512":
                for body in None, b"", b"request payload":
                    for stale_after in (None, 1, 4) if algorithm else (None,):
                        self._test_digest_auth(
                            username, password, qop, algorithm, body, stale_after
                        )
    ...
    def test_digest_auth_with_wrong_authorization_type(self):
        ...
        for header in auth_headers:
            response = self.app.get(
                "/digest-auth/auth/myname/mysecret", headers={header[0]: header[1]}
            )
            self.assertEqual(response.status_code, 401)

    def _test_digest_auth(
        self, username, password, qop, algorithm=None, body=None, stale_after=None
    ):
        uri = self._digest_auth_create_uri(
            username, password, qop, algorithm, stale_after
        )
        ...
        authorized_response, nonce = self._test_digest_response_for_auth_request(
            header, username, password, qop, uri, body
        )
        ...
        self._digest_auth_stale_after_check(
            header, username, password, uri, body, qop, stale_after
        )
    ...
    def _digest_auth_stale_after_check(
        self, header, username, password, uri, body, qop, stale_after
    ):
        for nc in range(2, stale_after + 1):
            authorized_response, nonce = self._test_digest_response_for_auth_request(
                header, username, password, qop, uri, body, nc
            )
            self.assertEqual(authorized_response.status_code, 200)
        stale_response, nonce = self._test_digest_response_for_auth_request(
            header, username, password, qop, uri, body, stale_after + 1
        )
    ...
    def _test_digest_response_for_auth_request(
        self, header, username, password, qop, uri, body, nc=1, nonce=None
    ):
        ...
        if qop:
            self.assertIn(
                qop,
                [
                    x.strip().removesuffix("'").removeprefix("'")
                    for x in d["qop"].removeprefix("(").removesuffix(")").split(",")
                ],
        ...
        cnonce, nc = (
            (_hash(os.urandom(10), "MD5"), "{:08}".format(nc))
            if qop in ("auth", "auth-int")
            else (None, None)
        )
    ...
    def test_digest_auth_wrong_pass(self):
        ...
        for qop in (
            ...
            for algorithm in None, "MD5", "SHA-256", "SHA-512":
                for body in None, b"", b"request payload":
                    self._test_digest_auth_wrong_pass(
                        username, password, qop, algorithm, body, 3
                    )

    def _test_digest_auth_wrong_pass(
        self, username, password, qop, algorithm=None, body=None, stale_after=None
    ):
        uri = self._digest_auth_create_uri(
            username, password, qop, algorithm, stale_after
        ...
        wrong_pass_response, nonce = self._test_digest_response_for_auth_request(
            header, username, "wrongPassword", qop, uri, body
        )
        ...
        reused_nonce_response, nonce = self._test_digest_response_for_auth_request(
            header, username, password, qop, uri, body, nonce=nonce
        )
    ...
    def test_request_range_with_parameters(self):
        response = self.app.get(
            "/range/100?duration=1.5&chunk_size=5", headers={"Range": "bytes=10-24"}
        )
    ...
    def test_parse_multi_value_header(self):
        ...
        self.assertEqual(
            parse_multi_value_header('"xyzzy", "r2d2xxxx", "c3piozzzz"'),
            ["xyzzy", "r2d2xxxx", "c3piozzzz"],
        )
        self.assertEqual(
            parse_multi_value_header('W/"xyzzy", W/"r2d2xxxx", W/"c3piozzzz"'),
            ["xyzzy", "r2d2xxxx", "c3piozzzz"],
        )
    ...
    def test_tags_get_all_tags(self):
        with (
            ...
            self.assertEqual(response.headers.get("X-Tag-Name"), "Tag Value")
            self.assertEqual(
                json.loads(response.data.decode("utf-8"))["GetTag_Name"], "GetTag Value"
            )
            self.assertEqual(
                json.loads(response.data.decode("utf-8"))["Another-Tag"],
                "Another-Tag Value",
            )
